...
if __name__ == '__main__':
    vid_name = '1.mp4'  # Здесь название видео, которое надо разбить на кадры
    ...
    # (Например 1, 10, 2. Вместо 1, 2, 10)
    for img_path in sorted(vid_frames_folder.glob('*.jpg'), key=lambda x: int(x.stem)):
        img = cv2.imread(img_path.as_posix())
        img = cv2.rotate(img,
                         cv2.ROTATE_90_COUNTERCLOCKWISE)  # Очень важно повернуть кадры, чтобы они сшивальсь
    ...
    num_to_stich = 10  # Количество склеиваемых фото. Чем больше, тем квадратично дольше ждать и менее стабильно. 10 практически оптимально.
