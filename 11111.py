...
def stitch_akaze(videofile,
    ...
    while grabbed:
        ...
        affine = None
        affine, status = cv2.findHomography(pts0, pts1, method=cv2.RANSAC, ransacReprojThreshold=ransac_threshold, maxIters=ransac_iters)
        ...
        min_x, min_y, max_x, max_y = border_points(transformations[0].dot(transformations[1]), images[1].shape[1], images[1].shape[0])
        print('Borders: min_x %s (0) min_y %s (0) max_x %s (%s) max_y %s ( % s) ' % (min_x, min_y, max_x, result.shape[1], max_y, result.shape[0]))
        result = cv2.copyMakeBorder(result, int(max(0, -min_y)), int(max(0, max_y-result.shape[0])), int(max(0, -min_x)), int(max(0, max_x-result.shape[1])), cv2.BORDER_CONSTANT, value= (0, 0, 0))

        _min_x, _min_y, _max_x, _max_y = border_points(transformations[0].dot(transformations[1]), images[1].shape[1], images[1].shape[0])
        shifts = np.array([-_min_x, -_min_y, _max_x-images[0].shape[1], _max_y])
        ...
        warped = cv2.warpPerspective(image1, transformations[1], (result.shape[1], result.shape[0]), result.copy(), borderMode = cv2.BORDER_TRANSPARENT)
...
stitch_akaze(videofile = 'trim-video-online.com_1711211909_3vmp4_e8VEVtqp.mp4',
                 kps_detect = 2000, #количество особых точек, которое ищем на кадрах
                 matches_take = 2000, #количество соответствий по которым строиться матрица
                 ransac_threshold = 4.0, #пороговое значение для алгоритма Ransac
                 ransac_iters=1000, #Количество итераций для алгоритма Ransac
                 fps = 60, #Число кадров в секунду
